...
class DensityMatrix:
    def __init__(self,sys_size=2):
        # a trajectory
        self.sys_size = sys_size
        self.rng = np.random.default_rng()

        rho = self.rng.random(size=self.sys_size)
        rho /= rho.sum() # eigenvalues
        D = self.rng.random(size=(self.sys_size,)*2)
        D -= D.T
        D = scipy.linalg.expm(D)
        self.rho = [np.einsum('ik,jk,k->ij',D,D,rho)]

        #rho_new = 
class Propagator:
    def __init__(self,sys_size=2,delta=1,decay=None):
        self.sys_size = sys_size
        self.delta = delta
        self.rng = np.random.default_rng()
    ...
    def propagate_rho(self,rhos):
        N = len(rhos)
        rho_new = rhos[N-1] - self.delta * 1j*(np.dot(self.Hs,rhos[N-1])-np.dot(rhos[N-1],self.Hs))
        #for m in range(N+1):
        #    rho_new += delta**2 * np.dot(Ks[N-m],self.rho[m-1])
        return rho_new
sys_size = 2
delta = 0.1
prop = Propagator(sys_size=sys_size,delta=delta)
dm = DensityMatrix(sys_size)
rho_new = prop.propagate_rho(dm.rho)
check_property(rho_new)
exit()
